...
describe("Testing API Transfer [Feelookup Fields]", () => {
    ...
    it("FeelookupFields[Produccion]",{
    }, () => {
      cy.fixture("data_test").then(function (datos) {
        this.datos = datos;
        cy.request({
          url: `${Cypress.env('API_TRANSFER_PRODUCCION')}/v1/transfers/feelookup/fields`,
          method: "POST",
          headers: { "Authorization": `${Cypress.env("AUTHORIZATION_TRANSFERS_PRODUCCION")}` },
          body: {   
            feelookupId: this.datos.idCotizacion_prd,
            quoteId: this.datos.idquote, 
          },
        }).then((response) => {
          expect(response.status).to.eq(200);
    ...
    it("FeelookupFields[Certificacion]",{
    }, () => {
      cy.fixture("data_test").then(function (datos) {
        this.datos = datos;
        cy.request({
